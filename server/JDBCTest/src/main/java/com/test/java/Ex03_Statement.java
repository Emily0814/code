...
public class Ex03_Statement {
	...
	private static void m5() {
		...
		String sql = String.format("insert into tblAddress (seq, name, age, gender, tel, address, regdate) " + "values (seqAddress.nextVal, '%s', '%s', '%s', '%s', '%s', default)", name, age, gender, tel, address); //여기에는 문장종결자를 쓸 수 없다.
	...
	private static void m1() {
		...
		try {
			...
			if(!conn.isClosed()) {
				...
				String sql = "insert into tblAddress (seq, name, age, gender, tel, address, regdate) values (seqAddress.nextVal, '호랑이', 20, 'm', '010-1234-5678','서울시 강남구 역삼동', default)"; //여기에는 문장종결자를 쓸 수 없다.
				
				stat = conn.createStatement();
				
				//반환값 없는 쿼리 > 오라클에게 전달되어 진행
