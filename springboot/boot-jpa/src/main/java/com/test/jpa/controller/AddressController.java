		...
		/*
			...
			2. JPQL
				...
				- 장점
					...
					- @Query 사용
				- 단점
					- 문자열로 작성 > 컴파일때 오류 확인 불가 > 런타임때 오류 확인 가능
					- 동적 쿼리 작성이 불편
...
//		}
	...
	public String m03(Model model) {
		...
		//CR[U]D
		//- 레코드 수정하기
		//1. 엔티티 새로 만든 뒤 > 이 값으로 기 존 레코드를 수정
		//1. 기존 엔티티 검색 > 기존 엔티티 수정 > 레코드를 수정
		
		//56	꿀꿀이	5	서울특별시 강남구 역삼동 한돈빌딩	m
...
//							.build();
	...
	public String m05(Model model) {
		...
		//Select문
		//- findBy, getBy, readBy, queryBy, streamBy 등..
		//- findBy(***)
		...
		//Optional<Address> address = addressRepository.findByAddress("서울특별시 강남구 학동로 202, 학동빌딩 8층");

		//Query did not return a unique result: 31 results were returned
...
//	}
	...
	public String m10(Model model) {
		...
		//After, Before, GreateThan(GreaterThanEqual), LessThan(LessThanEqual), Between
	...
	public String m14(Model model) {
		...
		Optional<Address> address = addressRepository.findByNameEquals("강아지"); //위에 3개가 동일한 것
	...
	public String m17(Model model) {
		...
		System.out.println(list.getTotalElements());	//50 > 총 엔티티수
	...
	public String m19(Model model, Pageable pageable) {
		...
		//m19.do?page=1&size=10&sort=name,desc
		//m19.do?page=2&size=10&sort=name,desc
	...
	public String m20(Model model, Pageable pageable) {
		...
		System.out.println(list.getNumberOfElements());		//가져온 데이터수
		...
		//Page request [number: 5, size 10, sort: UNSORTED]
		...
		System.out.println(list.isFirst());					//현재 페이지가 첫번째 페이지인지?
		System.out.println(list.isLast());					//현재 페이지가 마지막 페이지인지?
...
//		List<Address> list = addressRepository.listAll(dto); 
	...
	public String m25(Model model) {
		...
		//특정 컬럼 가져오기
		//- N개 컬럼
		//List<Address> list = customAddressRepository.findNameAndAgeAndGender();
	...
	public String m31(Model model) {
		...
		List<Tuple> list = customAddressRepository.findAllGeuopByGender();
