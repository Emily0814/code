...
select
    ...
    prior name as 부모부품명,
    level, --세대수
    connect_by_root name, --^최고계층
    connect_by_isleaf, --^최하위노드(자식구분)
    sys_connect_by_path(name, '→') --계층구조를 보여줌
from tblComputer
    --start with seq = 1  --최상위 레코드(루트 레코드) ^어디서부터 보고싶은지~
    --start with pseq is null --유연한 코드
