...
-- 15. 수도 이름이 '산'로 시작하는 국가의 이름과 수도를 조회하세요.
    ...
    select name, capital from tblCountry where capital like '산%'; --S로 시작하는 것이 없음
...
-- 18. 인구 정보가 없는 국가의 이름과 수도를 조회하세요.
    ...
    select name, continent from tblCountry where population is null;
...
-- 23. 키가 170cm 이상이고 몸무게가 80kg 이하인 코미디언의 이름과 키, 몸무게를 조회하세요.
    ...
    select first, last, height, weight from tblComedian where height >= 170 and weight <= 80;

-- 24. 이름(first)에 '동' 또는 '민'이 포함된 코미디언의 이름을 조회하세요.
    select * from tblComedian;
    
    select first, last from tblComedian where first like '%동%' or first like '%민%'; --재확인!!
...
-- 26. 키가 180cm 이상인 남성 코미디언의 이름과 키를 조회하세요.
    ...
    select first, height from tblComedian where height >= 180;
    
    
-- 27. 별명(nick)에 '상'이 포함된 코미디언의 이름과 별명을 조회하세요.
    select * from tblComedian;
    
    select first, nick from tblComedian where nick like '%상%';
    
-- 28. 몸무게가 70kg 이상 80kg 이하인 코미디언의 이름과 몸무게를 조회하세요.
    select * from tblComedian;
    
    select first, weight from tblComedian where weight between 70 and 80;
    
-- 29. 키가 160cm 이상 170cm 이하인 여성 코미디언의 이름과 키를 조회하세요.
    select * from tblComedian;
    
    select first, height from tblComedian where height between 160 and 170;
    
-- 30. 별명이 '프로'로 시작하는 코미디언의 이름과 별명을 조회하세요.
    select * from tblComedian;
    
    select first, nick from tblComedian where nick like '프로%';
...
-- 48. tblWomen에서 커플이 없는 여성의 이름과 나이를 조회하세요.
    ...
    select name, age from tblWomen where couple is not null;

-- 49. tblMen에서 이름이 '김'으로 시작하는 남성의 이름과 나이를 조회하세요.
    select * from tblMen, tblWomen;
    
    select name from tblMen where name like '김%';
...
-- 65. 아직 완료되지 않은 할 일 중 추가된 지 7일이 지난 항목의 제목과 추가된 날짜를 조회하세요.(X)
    select * from tblTodo;
    
    select * from tblTodo where completedate = null;   --추가된 지 7일이 지난 항목
...
-- 68. 완료된 할 일 중 추가된 날짜와 완료된 날짜가 같은 항목의 제목을 조회하세요.(풀이금지)
    select * from tblTodo;
        SELECT to_char(adddate, 'yyyy-mm-dd hh24:mi:ss'),
        to_char(adddate, 'yyyy-mm-dd hh24:mi:ss')
    FROM tblTodo
        WHERE to_char(adddate, 'yyyy-mm-dd') = to_char(completedate, 'yyyy-mm-dd');
    --select title from tblTodo where completedate is adddate;

-- 69. 아직 완료되지 않은 할 일을 추가된 날짜의 오름차순으로 조회하세요.
    select * from tblTodo;
    --SELECT *(전체) FROM 테이블명 ORDER BY 정렬할컬럼 ASC
    select adddate from tblTodo where completedate is null ORDER BY completedate asc;
...
-- 직업(job_id)이 프로그래머(it_prog)인 직원의 이름(풀네임)과 연락처 가져오시오.
    ...
    select last_name || first_name as name, phone_number from employees where job_id = 'IT_PROG';

-- 72.employees
-- last_name이 'Grant'인 직원의 이름, 연락처, 고용날짜를 가져오시오.
    select * from employees;

    select last_name || first_name as name, phone_number, hire_date from employees where last_name = 'Grant';

-- 73.employees
-- 특정 매니저(manager_id: 120)이 관리하는 직원의 이름, 급여, 연락처를 가져오시오.
    select * from employees;

    select last_name || first_name as name, salary, phone_number from employees where manager_id = 120;

-- 74.employees
-- 특정 부서(60, 80, 100)에 속한 직원들의 이름, 연락처, 이메일, 부서ID 가져오시오.
    select * from employees;

    select last_name || first_name as name, phone_number, email, job_id from employees where department_id in ( 60, 80, 100);
