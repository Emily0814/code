...
--    > 컬럼 리스트 > 숫자 컬럼 3개
    ...
    select max(length(first_name || last_name)) as 긴이름 from employees;
    select min(length(first_name || last_name)) as 짧은이름 from employees;
    select round(avg(length(first_name || last_name))) as 평균이름 from employees;
