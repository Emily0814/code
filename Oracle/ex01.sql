...
/*
    ...
    1. Oracle Database 11g Express Edition
        ...
        - 오라클(회사) > 오라클(제품)
        - 11g > 버전
        - Express Edition > 종류 > 무료 버전(성능 제한)
        - 실행 화면이 없는 프로그램 > 눈에 안보이는 프로그램 > 서비스, 데몬 
            > 조작 가능한 UI가 없다.
    ...
    1. 사용자 이름: system
    2. 비밀번호: java1234
    3. 호스트 이름: localhost > 오라클 설치된 컴퓨터 IP or 도메인
                                localhost(127.0.0.1) > 본인, 192.168.10.37 > IP주소
    4. 포트: 1521(Listener Port Number) > 외부로 부터 통하는 통로
    5. SID: 하나의 컴퓨터에 여러 오라클 설치 > 구분 식별자 > xe
    
    6. Name : 접속 이력명 > system@localhost
                            localhost.system(하위,멤버접근연산자,상위)
