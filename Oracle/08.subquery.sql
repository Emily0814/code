...
-- 1. employees. 'Munich' 도시에 위치한 부서에 소속된 직원들 명단?
    ...
    select location_id from locations where city = 'Munich'; --2700
    select department_id from departments where location_id = 2700; --70
    select job_id from employees where department_id = 70;
    
    select first_name || last_name from employees 
    where employees.job_id = ( select job_id from employees where department_id = 
