...
/*
    ...
    조인의 종류
    1. 단순 조인, Cross Join
    2. 내부 조인, Inner Join <연습
    3. 외부 조인, Outer Join <연습
    4. 셀프 조인, Self Join
    5. 젠체 외부 조인, Full Outer Join
...
select * from tblCustomer cross join tblSales; -- 표준 SQL > 권장!!, 3 x 9 = 17건, Cross join > *라고 생각하기
...
from tblStaff
    inner join tblProject
        on tblstaff.seq = tblProject.staff_seq; --^출력용 테이블

-- 고객번호, 고객명(tblCustomer), 상품번호, 상품명(tblSales)
select
    tblCustomer.seq as cseq,
    tblCustomer.name,
    tblSales.seq as sseq,
    tblSales.item
from tblCustomer
    inner join tblSales
        on tblCustomer.seq = tblSales.cseq;
...
-- 고객 + 판매
-- 어떤 고객(c.name)이 어떤 물건(s.item)을 몇개(s.qty) 사갔는지?
select
    c.name as 고객명,
    s.item as 상품명,
    s.qty as 수량
...
insert into tblCusrtomer values (4, '호호호', '010-9999-9999', '서울시'); --???
insert into tblCusrtomer values (5, '후후후', '010-5555-5555', '강릉시');


-- 내부 조인 > 9개
-- > 물건을 한번이라도 구매한 이력이 있는 고객의 정보 + 구매 내역을 가져오시오.
-- > 구매 이력이없는 4,5번 회원은 제외
...
-- 내부 조인 > 프로젝트를 최소 1건 이상 담당하고 있는 직원 명단 + 담당 프로젝트
select
    *
from tblStaff s
...
-- 회원() + 대여(C) > 대여를 최소 1회 이상 했던 회원 + 대여 내역
...
select
    ...
    --m.tel
    count(*),
    (select tel from tblMember where name = m.name) --상수로 치기 때문에 사용 가능
from tblMember m
    inner join tblRent r
        on m.seq = r.member
            group by m.name
...
select
    name as 직원명,
    department as 부서명,
    (select name from tblSelf where seq = a.super) as 상사명   --서브 쿼리 > 부모 테이블 > 상사명
