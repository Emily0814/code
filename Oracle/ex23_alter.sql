...
delete from tblEdit; --복구하는 가장 좋은 방법은 데이터 보관을 잘하자:)
...
-- Case 2. 컬럼을 삭제하기 > 커밋, 롤백으로도 복구 안됨
alter table 테이블명
    drop column 컬럼명;
...
-- Case 3. 컬럼을 수정하기
-- SQL 오류: ORA-12899: value too large for column "HR"."TBLEDIT"."NAME" (actual: 31, maximum: 20)
...
create table tblEdit (
    seq number,
    name varchar2(20) not null -- not null을 조작할 수 없음
);
...
insert into tblEdit values(1, '강아지'); --PK확인
insert into tblEdit values(1, '강아지');
insert into tblEdit values(2, '강아지'); --uq확인
