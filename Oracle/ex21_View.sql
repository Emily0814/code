...
create or replace view vwInsa
as
select * from tblInsa where buseo = '개발부'; --이 때는 select문이 실행되지 않음 > 재사용 뷰

update tblInsa set city = '제주' where num = 1060;

select * from vwInsa;   --복사본  > 수정
select * from (select * from tblInsa where buseo = '개발부'); --인라인 뷰 > 일회용 뷰
...
select
    ...
    round((sysdate - (r.rentdate + g.period)) * g.price * 0.1) || '원' as 연체료
from tblRent r
...
-- 신입 사원 > 담당 업무 > 전직원 문자 메세지 발송 > 전직원 연락처?
