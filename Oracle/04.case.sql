...
-- 4. 부서가 '영업부'면 5%, '총무부'면 3%, '개발부'면 7%, 나머지는 2%의 보너스율을 적용하세요.
    ...
    select
        buseo, basicpay,
        case
            when buseo = '영업부' then basicpay * 1.05
            when buseo = '총무부' then basicpay * 1.03
            when buseo = '개발부' then basicpay * 1.07
            else basicpay * 1.02
        end as 보너스율
    from tblInsa;
...
-- 8. 국가 이름의 길이가 4자 이상이면 '긴 이름', 그 외는 '짧은 이름'으로 분류하세요. 길이 함수: length(컬럼명)
    ...
    select 
        case
            when length(capital) >= 4 then '긴 이름'
            else '짧은 이름'
        end as 국가명길이
    from tblCountry;
...
-- 15. 코미디언의 나이(2024년 기준)를 계산하여 30세 이상이면 '중견', 그 외는 '신인'으로 분류하세요. (풀지마~~~~~)
    select * from tblComedian;
