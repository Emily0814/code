...
-- tblComedian. 몸무게 가장 많이 나가는 사람?
select * from tblComedian;
select max(weight) from tblComedian;
select * from tblComedian where weight = 129;

select * from tblComedian where weight = (select max(weight) from tblComedian); -- 값을 알려주는 select이면 뭐든 와도 됌.
...
select
    name, buseo, basicpay,
    (select round(avg(basicpay)) from tblInsa where buseo = a.buseo) as 평균급여
from tblInsa a;
