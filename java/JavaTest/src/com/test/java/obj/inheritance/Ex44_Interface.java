...
interface Keyboard {
	...
	//*** 인터페이스는 추상 메서드의 집합이다.
	//*** 인터페이스는 구현 멤버(변수, 일반 메서드)를 가질 수 없다.
	
	//public int a = 10; //The blank final field a may not have been initialized
	//public void test() {} //Abstract methods do not specify a body
	//public void test();
	
	//***** 키보드가 가져야할 행동 규법(규칙) > 추상 메서드
	//인터페이스는 public만 사용가능하므로 생략 가능
	void on();
	void off();
	void keydown();
	void keyup();
