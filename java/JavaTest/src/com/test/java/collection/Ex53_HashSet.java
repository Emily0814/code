...
public class Ex53_HashSet {
	...
	private static void m4() {
		...
		set2.add(new Member("홍길동", 20));   //여기서는 홍길동을 동명이인이라고 판단
											  //HashCode, equals 재정의 후 동일인물이라고 판단
		...
		Member m1 = new Member("홍길동", 20);	//instance["홍길동", 20] create from heap
		Member m2 = new Member("아무개", 22);	
		Member m3 = new Member("홍길동", 20);	//instance create 
		
		//주소값(참조형) 변수의 비교는 연산자 사용하지 말것 > equals
		System.out.println(m1 == m2);		//false
		System.out.println(m1.equals(m2));	//false
		System.out.println(m1.equals(m3));	//false, 주소값이 다름 -> true(equals오버로딩)
		
		System.out.println("---------------------------------------");
		
		System.out.println(m1); //com.test.java.collection.Member@782830e
		System.out.println("m1: " + m1.hashCode());	//1321640594 (메모리주소값)
		System.out.println("m3: " + m3.hashCode());	//457233904	 (메모리주소값)
...
//			//System.out.println(rnd.nextInt(10));		//오버로딩되어 있음, 0~9까지 return
...
//		System.out.println(iter.hasNext()); //읽어올 다음 데이터가 있냐?  	BOF 	0		1		2		EOF
//		System.out.println(iter.next());	//읽어라						set		사과	바나나	딸기	
//																			// 		  △(next())				  △(읽어올 것이 없음면 안됨)
...
//		System.out.println(iter.next());
//		
//		System.out.println(iter.hasNext());	//읽을 것이 없으면  false를 return
		
		
		 while (iter.hasNext()) {
			 
			 System.out.println(iter.next());	//순서가 따로 없기 때문에 넣었던 순서대로 출력되지 않음
			 									//순서가 맘에 안들면 쓰면 안됨
		 }
		 ...
		 while (iter.hasNext()) {
			 ...
			 System.out.println(iter.next());	//화살표는 여전히 끝에 있기 때문에 출력되지 않음
			 									//화살표는 전진만 가능하기 때문에 재사용 불가
