...
//		} 위에 3개의 식은 모두 true이다.
		...
		if (n > 0) {
			...
			System.out.println("양수 아님"); //{} 적는 것이 가독성? 굳굳
	...
	private static void m5() {
		...
		if (score >= 0 && score <= 100) {
			...
			} else{ System.out.println("점수는 0~100점 이내로 입력하세요.");
		} 
	...
	private static void m4() {
		...
		System.out.println("b. 1시간 이상 ~ 3시간 미만");		//+3
		...
		if (score >= 10) {
			System.out.println("당신을 개발자 자질이 있습니다.");
		} else if (score >= 5) {
	...
	private static void m3() {
		...
		if (hour < 6) {
			System.out.println("지하철을 타고 출근한다.");
		} else if(hour < 7) { //hour >= 6 && 이 부분은 좋은 코드가 아님 번거로움, 유지보수
			System.out.println("버스를 타고 출근한다.");
		} else if(hour < 8) {
			System.out.println("택시를 타고 출근한다.");
		} else {
