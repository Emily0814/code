...
public class Ex27_String {
	...
	public static void main(String[] args) {
		...
		//- boolean startsWith(String), boolean endsWidth(String)
	...
	private static void m11() {
		...
		System.out.println(txt.replace("홍길동","아무개")); //charsequence? 이게 String과 같은 것이라고 보면됨.
	...
	private static void m9() {
		...
		//문자열 추출
		//- char charAt(int)
		//- String substring(int beginIndex, int endIndex) > char보다 사용성이 우수
		...
		System.out.println(txt.substring(3, 8)); // 3~7
		System.out.println(txt.substring(5, 6)); // 5
		System.out.println(txt.substring(5)); // 끝자리 표시하지 않으면 끝까지 나옴
		
		char num2 = 'A';
		System.out.println((int)num2);
		
//		String num ="A";
//		System.out.println((int)num); //A 값형과 참조형은 형변환되지 않음
		...
		//몇년생?
		System.out.println(""+jumin.charAt(0) + jumin.charAt(1)); //char + char > 문자코드값이 나오기 때문에 97이 나오지 않음, but, "" + 해주면 해결됨
 		...
 		//파일 경로
 		String path = "C:\\class\\code\\java\\Hello.java"; // '\\' 한글자로 인식하기 때문에 c는 3으로 인식
	...
	private static void m8() {
		...
		System.out.println(name.endsWith("홍"));
		System.out.println();
...
//		System.out.println(txt.toLowerCase());
	...
	private static void m5() {
		...
		System.out.println(txt.indexOf("홍길동"));	//7, integer이기 때문에 시작위치를 반환해줌
		
		System.out.println(txt.indexOf('김'));	//-1, 가장 작은 값보다 작은값 && 0~13(문장숫자)외에 숫자를 출력
		
		txt = "안녕하세요. 홍길동입니다. 반갑습니다. 홍길동입니다. 안녕히가세요. 홍길동입니다.";
		
		System.out.println(txt.indexOf("홍길동")); //7 처음에 있는 홍길동만 검색됨
		System.out.println(txt.indexOf("홍길동",10)); //7 > 22
		System.out.println(txt.indexOf("홍길동",25)); //7 > 38 
		
		int index = 0; //0이 아닌 -1로 초기화해야함
		...
		//- int lastdexOf(char c)
		//- int lastdexOf(String s)
		//- int lastdexOf(char c, int beginIndex)
		//- int lastdexOf(String s, int beginIndex)
	...
	private static void m2() {
		...
		char c = txt.charAt(txt.length()-2); //-2 < 뒤에서 특정 번째 문자를 가져옴
...
//		
//		System.out.println("한글ABC123!@#".length()); //글자수를 알아내는 메서드
