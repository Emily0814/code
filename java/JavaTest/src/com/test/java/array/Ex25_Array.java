...
public class Ex25_Array {
	...
	private static void m13() {
		...
		System.out.println(Arrays.deepToString(nums2));	//2차원 배열 이상에서 사용
		System.out.println();
	...
	private static void m12() {
		...
		//- 내림차순(X)
		Arrays.sort(nums, 3, 7);	//3~6번째까지 정렬, 마지막은 포함안됨, 범위를 안주면 전체, 주면 부분정렬
		System.out.println(Arrays.toString(nums));
		
		Arrays.sort(names);
		System.out.println(Arrays.toString(names));
		
		//Arrays.sort(null, null); 클래스 수업도 같이 해야해서 나중에 내림차순 수업할 예정
	...
	private static void m10() {
		...
		for(int i=0; i<nums.length-1; i++) {//사이클(0,1,2,3)
			...
			for(int j=0; j<nums.length-1-i; j++) {
				...
				//2 vs 3
				//3 vs 4
				if(nums[j] < nums[j+1]) { //비교조건을 변경하면 내림차순으로 변경가능
					//swap
					temp = nums[j];
					nums[j] = nums[j+1];
					nums[j+1] = temp;
	...
	private static void m9() {
		...
		int[] nums3 = new int[] { 97, 54, 76, 88, 60};//길이 생략, 배열의 초기화
		int[] nums4 = new int[] { 97, 54, 76, 88, 60};
	...
	private static void m8() {
		...
		//배열 상태 확인?
		System.out.println(nums); //배열-> [ 자료형-> I @ 메모리 주소-> 5aaa6d82 출력:[I@5aaa6d82
		...
		//Arrays > 깊은 복사
		int[] copy = Arrays.copyOfRange(nums, 0, nums.length); //3 <- 0,1,2
	...
	private static void m7() {
		...
		//1. 얕은 복사, Shallow Copy > 기본 동작   사용빈도수 높음
		//- 주소값 복사
		...
		int[] copy = new int[3]; //1개라도 데이터를 기억하면 쓰레기가 아님, 필요없는 부분
...
//		copy[2] = nums[2];
	...
	private static void m6() {
		...
		int a = 10;
		changeValue(a);
		System.out.println("a: "+a); //값형 지역변수 복사본 영향x a: -> 10
		
		int[] b = new int[3];
		b[0] = 10;
		changeArray(b);
		System.out.println(b[0]); //참조형 지역변수 복사본 영향o -> 20
	...
	private static void m5() {
		...
		int[] nums = new int[3];
		nums[0] = 10; //nums(스택)->heap에 있는 nums[0]을 찾아 값을 가져옴, 모든 배열은 heap에서 생성
		nums[1] = 20; //nums(100), copy(200), copy = nums -> copy(100) 주소값이 복사됨
		nums[2] = 30;
		...
		//		 > Side Effect기 발생함.(주의)
		//int[] = int[] <- 자료형, integer배열
		copy = nums;
		
		nums[0] = 111;
		nums[1] = 222;
		nums[2] = 333; //참조형복사이기 때문에 원본을 변경하면 복사본도 같이 변경됨, 주소값을 확인해 heap에 있는 값을 가져옴.
...
//		Calendar c5;
	...
	private static void m3() {
		...
		//java.lang.ArrayIndexOutOfBoundsException: <- 방번호가 없는 방을 사용했음
