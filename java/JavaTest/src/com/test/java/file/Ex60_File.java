...
public class Ex60_File {
	...
	public static void main(String[] args) {
		...
		/*
			...
			저장 장치
			- 데이터 > 1, 0으로 저장
			- 저장 장치라는 환경 > 자료형 X  ^하드디스크에 저장된 모든 것은 파일이 없음, 문자열로 저장됨
	...
	private static void m9() {
		...
		File file = new File("dat\\score.txt"); // .\\ > 상대경로, C:\\ > 절대경로
		//C:\class\code\java\JavaTest\dat  같은 표현
		...
		if(file.exists()) {
			...
			System.out.println("===============================================");
			System.out.println("[이름]\t[국어]\t[영어]\t[수학]\t[수학]\t[평균]");
	...
	private static void m7() {
		...
		try {
			...
			line = reader.readLine();
			System.out.println(line); //읽을 것이 없으면 null 반환
...
//			System.out.println(c);
	...
	private static void m1() {
		...
		try {
			...
			//FileOutputStream
			//- 바이트 단위 쓰기 지원(1byte)
			FileOutputStream stream = new FileOutputStream(file, true);	//준비물 완료, 스트림 열기
			
//			char c = 'A';
//			
//			stream.write((int)c); //test.txt에 'A'를 적어라
//			stream.write(c); //자동 캐스팅
//			stream.write('M');
//			stream.write('O');
			...
			stream.close(); //스트림 닫기(파일 저장X) > 반드시 닫아야 함 왜? 저장되지 않음
